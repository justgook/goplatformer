...
// require *ebiten.Image sizes that exceed the GPUs maximum image size. If this were the case (i.e. if a level was upwards of
...
// LoadTileset simply loads a Tileset image from disk using ebitenutil.NewImageFromFile() function.
...
type RenderedLayer struct {
	...
	Layer *ldtk.Layer   // The layer used to render the image
}
...
// beginLayer gets called when necessary between rendering individual Layers of a Level.
func (er *EbitenRenderer) beginLayer(layer *ldtk.Layer, w, h int) {
...
// renderTile gets called by ldtk.Layer.RenderTiles(), and is currently provided the following arguments to handle rendering each tile in a Layer:
// x, y = position of the drawn tile
// srcX, srcY = position on the source tileset of the specified tile
...
func (er *EbitenRenderer) Render(level *ldtk.Level) {
	...
	for _, layer := range level.Layers {
		...
		case ldtk.LayerTypeIntGrid: // IntGrids get auto-tiles automatically
			fallthrough
		case ldtk.LayerTypeAutoTile:
			fallthrough
		case ldtk.LayerTypeTile:
