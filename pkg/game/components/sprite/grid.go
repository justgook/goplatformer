...
func assertPositiveInteger(value int, name string) {
	if value < 1 {
		log.Fatalf("%s should be a positive number, was %d", name, value)
	}
}

func assertSize(size, limit int, name string) {
	if size > limit {
		log.Fatalf("%s should be <= %d, was %d", name, limit, size)
	}
}
...
func (g *Grid) getOrCreateFrame(x, y int) *image.Rectangle {
	if x < 1 || x > g.width || y < 1 || y > g.height {
		log.Fatalf("There is no frame for x=%d, y=%d", x, y)
	}
	key := g.key
...
// Frames accepts an arbitrary number of parameters.
