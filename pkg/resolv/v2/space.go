...
type Space[T comparable] struct {
	Cells                 [][]*Cell[T]
...
func NewSpace[T comparable](spaceWidth, spaceHeight, cellWidth, cellHeight int) *Space[T] {
	sp := &Space[T]{
		CellWidth:  cellWidth,
...
func (sp *Space[T]) Add(objects ...*Object[T]) {
	if sp == nil {
...
// game.
func (sp *Space[T]) Remove(objects ...*Object[T]) {
	if sp == nil {
		panic("ERROR: space is nil")
	}
...
func (sp *Space[T]) Objects() []*Object[T] {
	objectsAdded := map[*Object[T]]bool{}
	var objects []*Object[T]
...
func (sp *Space[T]) Resize(width, height int) {
	sp.Cells = [][]*Cell[T]{}

	for y := 0; y < height; y++ {
		sp.Cells = append(sp.Cells, []*Cell[T]{})

		for x := 0; x < width; x++ {
			sp.Cells[y] = append(sp.Cells[y], newCell[T](x, y))
		}
	}
}
...
func (sp *Space[T]) Cell(x, y int) *Cell[T] {
...
func (sp *Space[T]) CheckCells(x, y, w, h int, tags []T) *Object[T] {
	for ix := x; ix < x+w; ix++ {
		for iy := y; iy < y+h; iy++ {
			cell := sp.Cell(ix, iy)
			if cell != nil {
...
func (sp *Space[T]) CheckCellsWorld(x, y, w, h float64, tags []T) *Object[T] {
...
func (sp *Space[T]) UnregisterAllObjects() {
...
func (sp *Space[T]) WorldToSpace(x, y float64) (int, int) {
...
func (sp *Space[T]) SpaceToWorld(x, y int) (float64, float64) {
...
func (sp *Space[T]) CellsInLine(startX, startY, endX, endY int) []*Cell[T] {
	var cells []*Cell[T]
