...
func testTouchClick(t *testing.T, flex *View, h *mockHandler, frame image.Rectangle) {
	...
	var tests = []struct {
		...
		{
			Scenario: "press inside right-bottom edge, release inside",
			Start:    frame.Max,
			End:      frame.Max,
			Want:     result{true, true, false},
		},
		{
			Scenario: "press inside right-bottom edge, release outside",
			Start:    frame.Max,
			End:      image.Pt(frame.Max.X+1, frame.Max.Y),
			Want:     result{true, true, true},
		},
...
func testMouseMove(t *testing.T, flex *View, h *mockHandler, frame image.Rectangle) {
	...
	var tests = []struct {
	...
	}{
		{
			...
			Point:    image.Point{X: frame.Min.X, Y: frame.Min.Y},
			Want:     result{IsMouseMoved: true, MousePoint: image.Point{X: frame.Min.X, Y: frame.Min.Y}},
		},
		{
			Scenario: "move mouse right-bottom inside",
			Point:    image.Point{X: frame.Max.X, Y: frame.Max.Y},
			Want:     result{IsMouseMoved: true, MousePoint: image.Point{X: frame.Max.X, Y: frame.Max.Y}},
		},
		{
			Scenario: "move mouse left outside",
			Point:    image.Point{X: frame.Min.X - 1, Y: frame.Min.Y},
			Want:     result{IsMouseMoved: false, MousePoint: image.Point{X: -1, Y: -1}},
		},
		{
			Scenario: "move mouse right outside",
			Point:    image.Point{X: frame.Max.X + 1, Y: frame.Min.Y},
			Want:     result{IsMouseMoved: false, MousePoint: image.Point{X: -1, Y: -1}},
		},
		{
			Scenario: "move mouse top outside",
			Point:    image.Point{X: frame.Min.X, Y: frame.Min.Y - 1},
			Want:     result{IsMouseMoved: false, MousePoint: image.Point{X: -1, Y: -1}},
		},
		{
			Scenario: "move mouse bottom outside",
			Point:    image.Point{X: frame.Min.X, Y: frame.Max.Y + 1},
			Want:     result{IsMouseMoved: false, MousePoint: image.Point{X: -1, Y: -1}},
		},
	}
...
func testSwipe(t *testing.T, flex *View, h *mockHandler, frame image.Rectangle) {
	...
	var tests = []struct {
		...
		{
			Scenario: "swipe left",
			From:     image.Point{X: frame.Min.X, Y: frame.Min.Y},
			To:       image.Point{X: frame.Min.X - 50, Y: frame.Min.Y},
		...
		},
		{
			Scenario: "swipe down",
			From:     image.Point{X: frame.Min.X, Y: frame.Min.Y},
			To:       image.Point{X: frame.Min.X, Y: frame.Min.Y + 50},
			Time:     time.Millisecond * 50,
		...
		},
		{
			...
			From:     image.Point{X: frame.Min.X, Y: frame.Min.Y},
			To:       image.Point{X: frame.Min.X, Y: frame.Min.Y + 49},
			Time:     time.Millisecond * 50,
			Want:     result{IsSwiped: false},
		},
	}
...
var _ Drawer = (*mockHandler)(nil)
var _ Updater = (*mockHandler)(nil)
...
func (h *mockHandler) Update(*View) {
	h.IsUpdated = true
}

func (h *mockHandler) Draw(_ *ebiten.Image, frame image.Rectangle, _ *View) {
	h.Frame = frame
	h.IsDrawn = true
}

func (h *mockHandler) HandlePress(_ int, _ int, _ ebiten.TouchID) {
	h.IsPressed = true
}

func (h *mockHandler) HandleRelease(_, _ int, isCancel bool) {
	h.IsReleased = true
	h.IsCancel = isCancel
}
