...
func (ct *containerEmbed) drawChild(screen *ebiten.Image, child *children) {
...
func (ct *containerEmbed) computeBounds(child *children) image.Rectangle {
	if child.absolute {
		return child.bounds
	}
...
func (ct *containerEmbed) handleDraw(screen *ebiten.Image, b image.Rectangle, child *children) {
	if h, ok := child.item.Handler.(Drawer); ok {
		h.Draw(screen, b, child.item)
	}
}

func (ct *containerEmbed) shouldDrawChild(child *children) bool {
	return !child.item.Hidden && child.item.Display != DisplayNone && child.item.Handler != nil
}

func (ct *containerEmbed) debugDraw(screen *ebiten.Image, b image.Rectangle, child *children) {
	if Debug {
		pos := fmt.Sprintf("(%d, %d)-(%d, %d):%s", b.Min.X, b.Min.Y, b.Max.X, b.Max.Y, child.item.ID)
...
func (ct *containerEmbed) handleMouseButtonLeftPressed(x, y int) bool {
	...
	for c := len(ct.children) - 1; c >= 0; c-- {
		...
		if mouseLeftClickHandler, ok := child.item.Handler.(MouseLeftButtonHandler); ok {
		...
		if button, ok := child.item.Handler.(ButtonHandler); ok {
			for {
				if button2, ok3 := child.item.Handler.(NotButton); ok3 {
					if !button2.IsButton() {
						break
					}
				}
...
func (ct *containerEmbed) childFrame(c *children) *image.Rectangle {
	if !c.absolute {
