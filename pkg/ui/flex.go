// Package ui referenced code: https://github.com/golang/exp/blob/master/shiny/widget/flex/flex.go
package ui
...
func (f *flexEmbed) layout(width, height int, container *containerEmbed) {
	...
	if f.Wrap == NoWrap {
		// Single line
		line := flexLine{child: make([]*element, len(nodes))}
		for i := range nodes {
			child := &nodes[i]
		...
		if len(line.child) > 0 || len(nodes) == 0 {
			lines = append(lines, line)
		}
	}
	...
	if len(lines) == 1 {
		...
		for l := range lines {
			...
			high := 0.0
			for _, child := range line.child {
				if child.crossSize > high {
					high = child.crossSize +
	...
	remCrossSize := containerCrossSize - offset
	...
	for _, line := range lines {
		...
		high := -1.
		for _, child := range line.child {
			if child.crossOffset < low {
				low = child.crossOffset
			}
			if high == -1 || child.crossOffset+child.crossSize > high {
				high = child.crossOffset + child.crossSize
			}
		}
		if intrinsicCrossSize < high-low {
			intrinsicCrossSize = high - low
		}
	}
	...
	// Layout complete. Update nodes position
	for l := range lines {
		line := &lines[l]
		for _, child := range line.child {
			switch f.Direction {
...
func (f *flexEmbed) mainMargin(c *children) []float64 {
	switch f.Direction {
	case Row:
		return []float64{
...
func (f *flexEmbed) crossMargin(c *children) []float64 {
	switch f.Direction {
	case Row:
		return []float64{
			float64(c.item.MarginTop),
...
func (f *flexEmbed) flexBaseSize(c *children) int {
	w := c.item.Width
	if w == 0 {
